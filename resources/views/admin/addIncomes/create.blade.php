...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.addIncome.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.add-incomes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('income_type') ? 'is-invalid' : '' }}" name="income_type" id="income_type" required>
                    ...
                    @foreach(App\Models\AddIncome::INCOME_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('income_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('source_type') ? 'is-invalid' : '' }}" name="source_type_id" id="source_type_id" required>
                    @foreach($source_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('source_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
