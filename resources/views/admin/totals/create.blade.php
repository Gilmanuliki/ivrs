...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.total.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.totals.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('nationality') ? 'is-invalid' : '' }}" name="nationality" id="nationality" required>
                    ...
                    @foreach(App\Models\Total::NATIONALITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('nationality', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
